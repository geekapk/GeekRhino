...
public class Bug491621Test {
    ...
    /**
     * Asserts that the value returned by {@link AstRoot#toSource()} after the given input source
     * was parsed equals the specified expected output source.
     *
     * @param source the JavaScript source to be parsed
     * @param expectedOutput the JavaScript source that is expected to be returned by {@link
     *     AstRoot#toSource()}
     */
    ...
    public void testVarDeclarationToSource() {
        ...
        assertSource(
                ...
                "for (var i = 0; i < 10; i++) \n  var x = i;\nx++;\n");
        assertSource(
                "function f(){var i=2;for(var j=0;j<i;++j)print(j);}",
                "function f() {\n  var i = 2;\n  for (var j = 0; j < i; ++j) \n    print(j);\n}\n");
    }
    ...
    public void testLetDeclarationToSource() {
        ...
        assertSource(
                "switch(x){case 1:let y;z++}",
                "switch (x) {\n  case 1:\n    let y;\n    z++;\n}\n");
        assertSource("for(let p in o)s+=o[p]", "for (let p in o) \n  s += o[p];\n");
        assertSource("if(c)let a=0;else a=1", "if (c) \n  let a = 0;\nelse \n  a = 1;\n");
        assertSource(
                "for(let i=0;i<10;i++){let x=i;}x++;",
                "for (let i = 0; i < 10; i++) {\n  let x = i;\n}\nx++;\n");
        assertSource(
                "function f(){let i=2;for(let j=0;j<i;++j)print(j);}",
                "function f() {\n  let i = 2;\n  for (let j = 0; j < i; ++j) \n    print(j);\n}\n");
    }
