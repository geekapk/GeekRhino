...
public final class BytecodeArray {
     ...
     *       {@code type} argument set to indicate the actual type. E.g., {@code aload} becomes
     *       {@code iload}, but {@code type} is passed as {@code Type.OBJECT} in that case.
     *   <li>Numeric conversion ops ({@code i2l}, etc.) are left alone to avoid too much confustion,
     *       but their {@code type} is the pushed type. E.g., {@code i2b} gets type {@code
     *       Type.INT}, and {@code f2d} gets type {@code Type.DOUBLE}. Other unaltered opcodes also
     *       get their pushed type. E.g., {@code arraylength} gets type {@code Type.INT}.
     * </ul>
     *
                ...
                case ByteOps.BIPUSH:
                    {
                        ...
                        visitor.visitConstant(
                                ByteOps.LDC, offset, 2, CstInteger.make(value), value);
                        return 2;
                    }
                case ByteOps.SIPUSH:
                    {
                        ...
                        visitor.visitConstant(
                                ByteOps.LDC, offset, 3, CstInteger.make(value), value);
                        return 3;
                    }
                case ByteOps.LDC:
                    {
    ...
    public interface Visitor {
        ...
        /**
         * Visits an instruction which has a (possibly synthetic) constant argument, and possibly
         * also an additional literal integer argument. In the case of {@code multianewarray}, the
         * argument is the count of dimensions. In the case of {@code invokeinterface}, the argument
         * is the parameter count or'ed with the should-be-zero value left-shifted by 8. In the case
         * of entries of type {@code int}, the {@code value} field always holds the raw value (for
         * convenience of clients).
         *
         * <p><b>Note:</b> In order to avoid giving it a barely-useful visitor all its own, {@code
         * newarray} also uses this form, passing {@code value} as the array type code and {@code
         * cst} as a {@link CstType} instance corresponding to the array type.
         *
         * @param opcode the opcode
         * @param offset offset to the instruction
         * @param length length of the instruction, in bytes
        ...
        public void visitNewarray(
                int offset, int length, CstType type, ArrayList<Constant> initVals);
