...
public final class Frame {
     ...
     * @param locals {@code non-null;} the locals array to use
     * @param stack {@code non-null;} the execution stack to use
     * @param subroutines {@code non-null;} list of subroutine start labels for subroutines this
     *     frame is nested in
     */
    ...
    /**
     * Replaces all the occurrences of the given uninitialized type in this frame with its
    ...
    /**
     * Returns the largest subroutine nesting this block may be in. An empty list is returned if
     * this block is not in any subroutine. Subroutines are identified by the label of their start
     * block. The list is ordered such that the deepest nesting (the actual subroutine this block is
     * in) is the last label in the list.
     *
    ...
    /**
     * Returns a Frame instance representing the frame state that should be used when returning from
     * a subroutine. The stack state of all subroutine invocations is identical, but the locals
     * state may differ.
     *
    ...
    private IntList mergeSubroutineLists(IntList otherSubroutines) {
        ...
        for (int i = 0;
                i < szSubroutines
                        && i < szOthers
                        && (subroutines.get(i) == otherSubroutines.get(i));
                i++) {
     ...
     * LocalsArraySet that will need to be trimmed of all OneLocalsArray elements that relevent to
        ...
        if (subroutines.equals(newOtherSubroutines)) {
            ...
            for (int i = szNonResult - 1; i >= 0; i--) {
                if (nonResultSubroutines.get(i)
                        != resultSubroutines.get(i + (szResult - szNonResult))) {
