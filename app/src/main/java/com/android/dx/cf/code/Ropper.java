...
public final class Ropper {
    ...
    /**
     * {@code non-null;} for each block (by label) that is used as an exception handler in the
     * input, the exception handling info in Rop.
     */
    ...
    private class Subroutine {
        ...
        /**
         * Generates a list of subroutine successors. Note: successor blocks could be listed more
         * than once. This is ok, because this successor list (and the block it's associated with)
         * will be copied and inlined before we leave the ropper. Redundent successors will result
         * in redundent (no-op) merges.
         *
    ...
    public static RopMethod convert(
            ConcreteMethod method, TranslationAdvice advice, MethodList methods) {
        try {
    ...
    /**
     * Gets the label for the given special-purpose block. The given label should be one of the
     ...
     *
     * <ul>
     *   <li>[0, maxLabel[ are the labels of the blocks directly corresponding to the input
     *       bytecode.
     ...
     *       synch blocks.
     *   <li>[maxLabel method.getCatches().size() + SPECIAL_LABEL_COUNT, getAvailableLabel()[
     *       assigned labels. Note that some of the assigned labels may not be used any more if they
     *       were assigned to a block that was deleted since.
     * </ul>
     *
    ...
    /**
     * Gets the register spec to use to hold the object to synchronize on, for a synchronized
     * method.
     *
    ...
    /**
     * Helper for {@link #addOrReplaceBlock} which recursively removes the given block and all
     * blocks that are (direct and indirect) successors of it whose labels indicate that they are
     * not in the normally-translated range.
     *
    ...
    private void processBlock(ByteBlock block, Frame frame, int[] workSet) {
        ...
        /*extraBlockCount*/ ; extraBlockCount > 0; extraBlockCount--) {
            ...
            if (needsGoto) {
                il.set(
                        1,
                        new PlainInsn(
                                Rops.GOTO, extraInsn.getPosition(), null, RegisterSpecList.EMPTY));
                /*
        ...
        if ((lastInsn == null) || (lastInsn.getOpcode().getBranchingness() == Rop.BRANCH_NONE)) {
            SourcePosition pos =
                    (lastInsn == null) ? SourcePosition.NO_INFO : lastInsn.getPosition();
    ...
    private void mergeAndWorkAsNecessary(
        ...
        if (existing != null) {
            ...
            if (calledSubroutine != null) {
                merged =
                        existing.mergeWithSubroutineCaller(
                                frame, calledSubroutine.getStartBlock(), pred);
            } else {
    ...
    private void addSetupBlocks() {
        ...
        BasicBlock bb =
                new BasicBlock(
                        getSpecialLabel(PARAM_ASSIGNMENT), insns,
        ...
        if (synch) {
            ...
            if (isStatic()) {
                insns.set(
                        0,
                        new PlainInsn(
                                Rops.opMoveResultPseudo(synchReg),
                                pos,
                                synchReg,
                                RegisterSpecList.EMPTY));
            }
    ...
    private void addReturnBlock() {
        ...
        if (isSynchronized()) {
            ...
            BasicBlock bb =
                    new BasicBlock(label, insns, IntList.makeImmutable(nextLabel), nextLabel);
            addBlock(bb, IntList.EMPTY);
    ...
    /**
     * Constructs and adds, if necessary, the catch-all exception handler block to deal with
     * unwinding the lock taken on entry to a synchronized method.
     */
    private void addSynchExceptionHandlerBlock() {
        ...
        insn =
                new PlainInsn(
                        Rops.opMoveException(Type.THROWABLE), pos, exReg, RegisterSpecList.EMPTY);
        insns.set(0, insn);
        insn =
                new ThrowingInsn(
                        Rops.MONITOR_EXIT,
        ...
        bb =
                new BasicBlock(
                        getSpecialLabel(SYNCH_CATCH_1), insns,
    ...
    private class SubroutineInliner {
        ...
        SubroutineInliner(
                final LabelAllocator labelAllocator, ArrayList<IntList> labelToSubroutines) {
         ...
         * label and adding it to the worklist if necessary. If the origLabel is a "special" label,
         * it is returned exactly and not scheduled for duplication: copying never proceeds past a
         * special label, which likely is the function return block or an immediate predecessor.
         *
    ...
    /**
     * Visits each block once in depth-first successor order, ignoring {@code jsr} targets. Worker
     * for {@link #forEachNonSubBlockDepthFirst}.
     *
