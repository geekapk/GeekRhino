...
/*package*/ final class RopperMachine extends ValueAwareMachine {
    ...
    /**
     * If non-null, the ReturnAddress that was used by the terminating ret instruction. If null,
     * there was no ret instruction encountered.
     */
    ...
    /**
     * Gets ready to start working on a new block. This will clear the {@link #insns} list, set
     * {@link #catches}, reset whether it has been used, reset whether the block contains a {@code
     * return}, and reset {@link #primarySuccessorIndex}.
     */
    ...
    /**
     * Gets the primary successor index. This is the index into the successors list where the
     * primary may be found or {@code -1} if there are successors but no primary successor. This may
     * return something other than {@code -1} in the case of an instruction with no successors at
     * all (primary or otherwise).
     *
    ...
    /**
     * @return {@code null-ok;} return address of a {@code ret} instruction if encountered since
     *     last call to startBlock(). {@code null} if no ret instruction encountered.
     */
    ...
    public void run(Frame frame, int offset, int opcode) {
        ...
        if (destCount == 0) {
            ...
            for (int pattern = getAuxInt(); pattern != 0; pattern >>= 4) {
                ...
                insns.add(
                        new PlainInsn(
                                Rops.opMove(type), pos, scratch.withReg(stackPointer), scratch));
        ...
        if (dest != null && rop.isCallLike()) {
            ...
            moveResult =
                    new PlainInsn(
                            Rops.opMoveResult(((CstMethodRef) cst).getPrototype().getReturnType()),
                            pos,
                            dest,
                            RegisterSpecList.EMPTY);
        ...
        if (cases != null) {
            ...
            if (sources.size() != 0) {
                ...
                if (source.getReg() != 0) {
                    insns.add(
                            new PlainInsn(
                                    Rops.opMove(type), pos, RegisterSpec.make(0, type), source));
                }
            }
            insn = new PlainInsn(Rops.GOTO, pos, null, RegisterSpecList.EMPTY);
            primarySuccessorIndex = 0;
     ...
     * @param stackPointer {@code >= 0;} the stack pointer after the instruction's arguments have
     *     been popped
