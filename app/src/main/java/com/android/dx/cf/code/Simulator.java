...
public class Simulator {
    ...
    /**
     * Simulates the effect of the instruction at the given offset, by making appropriate calls on
     * the given frame.
     *
     ...
     *
     * <p>In the case of arrays of objects, we want to narrow the type to the actual array present
     * on the stack, as long as what is present is an object type. Similarly, due to a quirk of the
     ...
     *
     * <p>In the case where there is a known-null on the stack where an array is expected, we just
     * fall back to the implied type of the instruction. Due to the quirk described above, this
     * means that source code that uses <code>boolean[]</code> might get translated surprisingly --
     * but correctly -- into an instruction that specifies a <code>byte[]</code>. It will be
     * correct, because should the code actually execute, it will necessarily throw a <code>
     * NullPointerException</code>, and it won't matter what opcode variant is used to achieve that
     * result.
     *
    ...
    private class SimVisitor implements BytecodeArray.Visitor {
        /**
         * {@code non-null;} machine instance to use (just to avoid excessive cross-object field
         * access)
         */
        private final Machine machine;
        ...
        /**
         * Checks whether the prototype is compatible with returning the given type, and throws if
         * not.
         *
