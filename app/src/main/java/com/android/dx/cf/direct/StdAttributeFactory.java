...
public class StdAttributeFactory extends AttributeFactory {
    ...
    private Attribute code(DirectClassFile cf, int offset, int length, ParseObserver observer) {
        ...
        if (observer != null) {
            observer.parsed(
                    bytes, offset, 2, "exception_table_length: " + Hex.u2(exceptionTableLength));
        }
    ...
    private Attribute enclosingMethod(
        ...
        if (observer != null) {
            ...
            observer.parsed(
                    bytes, offset + 2, 2, "method: " + DirectClassFile.stringOrNone(method));
        }
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (observer != null) {
                observer.parsed(
                        ...
                        offset,
                        2,
                        "inner_class: " + DirectClassFile.stringOrNone(innerClass));
                observer.parsed(
                        bytes,
                        offset + 2,
                        2,
                        "  outer_class: " + DirectClassFile.stringOrNone(outerClass));
                observer.parsed(
                        bytes, offset + 4, 2, "  name: " + DirectClassFile.stringOrNone(name));
                observer.parsed(
                        bytes,
                        offset + 6,
                        2,
                        "  access_flags: " + AccessFlags.innerClassString(accessFlags));
            }
            offset += 8;
        }
    ...
    private Attribute localVariableTable(
        ...
        LocalVariableList list =
                parseLocalVariables(
                        bytes.slice(offset + 2, offset + length),
                        cf.getConstantPool(),
                        observer,
                        count,
                        false);
     ...
     *
     * @param bytes {@code non-null;} bytes to parse, which should <i>only</i> contain the table
     *     data (no header)
