...
public class Main {
    ...
    public static String getTooManyIdsErrorMessage() {
        if (args.multiDex) {
            return "The list of classes given in "
    ...
    /**
     * Constructs the output {@link DexFile}, fill it in with all the specified classes, and
     * populate the resources map if required.
     *
    ...
    private static boolean processAllFiles() {
        ...
        try {
            if (args.mainDexListFile != null) {
                ...
                FileNameFilter mainPassFilter =
                        args.strictNameCheck
                                ? new MainDexListFilter()
        ...
        if (args.numThreads > 1) {
            ...
            try {
                ...
                // If we're here, it means all threads have completed cleanly, so there should not
                // be
        ...
        if (errorNum != 0) {
            DxConsole.err.println(
                    errorNum + " error" + ((errorNum == 1) ? "" : "s") + "; aborting");
            return false;
        }
        ...
        opener =
                new ClassPathOpener(
                        ...
                        new ClassPathOpener.Consumer() {
                            ...
                            @Override
                            public boolean processFileBytes(
                                    String name, long lastModified, byte[] bytes) {
    ...
    private static boolean processClass(String name, byte[] bytes) {
        ...
        int maxMethodIdsInDex =
                numMethodIds
                        + constantPoolSize
                        + cf.getMethods().size()
                        + MAX_METHOD_ADDED_DURING_DEX_CREATION;
        int maxFieldIdsInDex =
                numFieldIds
                        + constantPoolSize
                        + cf.getFields().size()
        ...
        if (args.multiDex
            ...
            createDexFile();
            assert (completeDex.getMethodIds().items().size()
                            <= numMethodIds + MAX_METHOD_ADDED_DURING_DEX_CREATION)
                    && (completeDex.getFieldIds().items().size()
                            <= numFieldIds + MAX_FIELD_ADDED_DURING_DEX_CREATION);
        }
    ...
    /**
     * Check the class name to make sure it's not a "core library" class. If there is a problem,
     * this updates the error count and throws an exception to stop processing.
     *
     ...
     * .jar} file and for checking against a classfile-internal "this class" name. This looks for
     * the last instance of the substring {@code "/./"} within the path, and if it finds it, it
     * takes the portion after to be the fixed path. If that isn't found but the path starts with
     * {@code "./"}, then that prefix is removed and the rest is return. If neither of these is the
     * case, this method returns its argument.
     *
     * @param path {@code non-null;} the path to "fix"
     * @return {@code non-null;} the fixed version (which might be the same as the given {@code
     *     path})
     */
    ...
    public static class Arguments {
        ...
        /**
         * whether to force generation of const-string/jumbo for all indexes, to allow merges
         * between dex files with many strings.
         */
        ...
        private static class ArgumentsParser {
            ...
            /**
             * Moves on to the next argument. Returns false when we ran out of arguments that start
             * with --.
             */
            ...
            /**
             * Checks the current argument against the given prefix. If prefix is in the form
             * '--name=', an extra value is expected. The argument can then be in the form
             * '--name=value' or as a 2-argument form '--name value'.
             */
        ...
        public void parse(String[] args) {
            ...
            while (parser.getNext()) {
                if (parser.isArg("--debug")) {
                    ...
                    if (dontOptimizeListFile != null) {
                        System.err.println(
                                "--optimize-list and " + "--no-optimize-list are incompatible.");
                        throw new UsageException();
                    }
                    optimize = true;
            ...
            if (multiDex && numThreads != 1) {
                System.out.println(
                        NUM_THREADS_OPTION + " is ignored when used with " + MULTI_DEX_OPTION);
                numThreads = 1;
            }

            if (multiDex && incremental) {
                System.err.println(
                        INCREMENTAL_OPTION + " is not supported with " + MULTI_DEX_OPTION);
                throw new UsageException();
            }
        ...
        /** Copies relevent arguments over into CfOptions and DexOptions instances. */
