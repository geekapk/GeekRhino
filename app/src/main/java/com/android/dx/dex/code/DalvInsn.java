...
public abstract class DalvInsn {
    ...
    public static SimpleInsn makeMove(
            SourcePosition position, RegisterSpec dest, RegisterSpec src) {
    ...
    /**
     * Constructs an instance. The output address of this instance is initially unknown ({@code
     * -1}).
     *
     * <p><b>Note:</b> In the unlikely event that an instruction takes absolutely no registers
     * (e.g., a {@code nop} or a no-argument no-result static method call), then the given register
     * list may be passed as {@link RegisterSpecList#EMPTY}.
     *
     ...
     * expanded transformation of this instance, and it is guaranteed that the number of low
     * registers used will be the number returned by {@link #getMinimumRegisterRequirement}.
     *
     * @param compatRegs {@code non-null;} set of compatible registers
     * @return {@code non-null;} the replacement
     */
     ...
     *
     * @param prefix {@code non-null;} prefix before the address; each follow-on line will be
     *     indented to match as well
     * @param width {@code >= 0;} the width of the output or {@code 0} for unlimited width
     * @param noteIndices whether to include an explicit notation of constant pool indices
     * @return {@code null-ok;} the string form or {@code null} if this instance should not appear
     *     in a listing
     */
     ...
     *
     * @return {@code null-ok;} the string version of any arguments or {@code null} if there are
     *     none
     */
    protected abstract String argString();

    /**
     * Helper for {@link #listingString}, which returns the string form of this instance suitable
     * for inclusion in a human-oriented listing dump, not including the instruction address and
     * without respect for any output formatting. This method should return {@code null} if this
