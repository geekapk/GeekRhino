...
public final class LocalList extends FixedSizeList {
    ...
    public static class Entry implements Comparable<Entry> {
        ...
        /**
         * Compares by (in priority order) address, end then start disposition (variants of end are
         * all consistered equivalent), and spec.
         *
    ...
    private static void debugVerify0(LocalList locals) {
        ...
        for (int i = 0; i < sz; i++) {
            ...
            if (e.isStart()) {
                ...
                if (active[reg] == null) {
                    throw new RuntimeException(
                            "redundant end at " + Integer.toHexString(e.getAddress()));
                }
                    ...
                    if (test.getRegisterSpec().getReg() == reg) {
                        if (test.isStart()) {
                            if (e.getDisposition() != Disposition.END_REPLACED) {
                                throw new RuntimeException(
                                        "improperly marked end at " + Integer.toHexString(addr));
                            }
                            foundStart = true;
                        } else {
                            throw new RuntimeException(
                                    "redundant end at " + Integer.toHexString(addr));
                        }
                    }
                }
    ...
    public static class MakeState {
        ...
        /**
         * Helper for {@link #endLocal}, which handles the cases where and end local is issued at
         * the same address as a start local for the same register. If this case is found, then this
         * method will remove the start (as the local was never actually active), update the {@link
         * #endIndices} to be accurate, and if needed update the newly-active end to reflect an
         * altered disposition.
         *
         * @param address {@code >= 0;} the address
         * @param endedLocal {@code non-null;} spec representing the local being ended
        ...
        /**
         * Converts a given spec into the form acceptable for use in a local list. This, in
         * particular, transforms the "known null" type into simply {@code Object}. This method
         * needs to be called for any spec that is on its way into a locals list.
         *
         ...
         * @return {@code null-ok;} an appropriately modified spec, or the original if nothing needs
         *     to be done
         */
        ...
        /**
         * Adds or updates an end local (changing its disposition). If this would cause an empty
         * range for a local, this instead removes the local entirely.
         *
         * @param address {@code >= 0;} the address
         * @param disposition {@code non-null;} the disposition
         * @param spec {@code non-null;} spec representing the local
