...
public final class OutputFinisher {
    ...
    /**
     * Helper for {@link #add} which scrutinizes a single instruction for local variable
     * information.
     *
    ...
    /**
     * Helper for {@link #getAllConstants} which adds all the info for a single {@code
     * RegisterSpec}.
     *
     * @param result {@code non-null;} result set to add to
    ...
    /**
     * Reverses a branch which is buried a given number of instructions backward in the output. It
     * is illegal to call this unless the indicated instruction really is a reversible branch.
     *
     ...
     *   <li>rewriting instructions, because of register number, constant pool index, or branch
     *       target size issues
     ...
     * are required and then reserving them. It also updates the given {@code opcodes} array so as
     * to avoid extra work when constructing the massaged instruction list.
     *
    ...
    /**
     * Attempts to fit the given instruction into a specific opcode, returning the opcode whose
     * format that the instruction fits into or {@code null} to indicate that the instruction will
     * need to be expanded. This fitting process starts with the given opcode as a first "best
     * guess" and then pessimizes from there if necessary.
     *
     * @param insn {@code non-null;} the instruction in question
     * @param guess {@code null-ok;} the current guess as to the best opcode; {@code null} means
     *     that no simple opcode fits
     ...
     *
     * <p>This method ignores issues of branch target size, since final addresses aren't known at
     * the point that this method is called.
     *
     * @param opcodes {@code non-null;} array of per-instruction opcode selections
     */
    ...
    /**
     * Helper for {@link #massageInstructions}, which constructs a replacement list, where each
     * {link DalvInsn} instance that couldn't be represented simply (due to register representation
     ...
     * possibly rewriting branches to fix ones that wouldn't otherwise be able to reach their
     * targets.
     */
     ...
     * this rewrites them to use a {@code goto} of some sort. In the case of a conditional branch
     * that doesn't fit, the sense of the test is reversed in order to branch around a {@code goto}
    ...
    private void shiftAllRegisters(int delta) {
        ...
        for (int i = 0; i < insnSize; i++) {
            DalvInsn insn = insns.get(i);
            // Since there is no need to replace CodeAddress since it does not use registers, skips
            // it to
