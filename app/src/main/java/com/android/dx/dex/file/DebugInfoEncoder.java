...
public final class DebugInfoEncoder {
     ...
     * @param file {@code null-ok;} may only be {@code null} if simply using this class to do a
     *     debug print
     * @param codeSize
     * @param regSize
     * @param isStatic
    ...
    /**
     * Gets the register that begins the method's parameter range (including the 'this' parameter
     * for non-static methods). The range continues until {@code regSize}
     *
    ...
    /**
     * Returns a string representation of this LocalList entry that is appropriate for emitting as
     * an annotation.
     *
    ...
    private void emitLocalRestart(LocalList.Entry entry) throws IOException {
        ...
        if (annotateTo != null || debugPrint != null) {
            annotate(
                    output.getCursor() - mark,
                    String.format(
                            "%04x: +local restart %s", address, entryAnnotationString(entry)));
        }
    ...
    /**
     * Emits a type index as an unsigned LEB128. The actual value written is shifted by 1, so that
     * the '0' value is reserved for "null".
     *
    ...
    /**
     * Emits a {@link DebugInfoConstants#DBG_START_LOCAL_EXTENDED DBG_START_LOCAL_EXTENDED}
     * sequence.
     *
     * @param entry entry to emit
     * @throws IOException
    ...
    /**
     * Emits the necessary byte sequences to emit the given position table entry. This will
     * typically be a single special opcode, although it may also require DBG_ADVANCE_PC or
     * DBG_ADVANCE_LINE.
     *
     ...
     *
     * @param deltaLines {@code >= DBG_LINE_BASE, <= DBG_LINE_BASE + DBG_LINE_RANGE;} the line
     *     change to encode
