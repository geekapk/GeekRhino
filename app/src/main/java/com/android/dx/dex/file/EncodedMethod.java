...
public final class EncodedMethod extends EncodedMember implements Comparable<EncodedMethod> {
     ...
     * @param code {@code null-ok;} code for the method, if it is neither {@code abstract} nor
     *     {@code native}
     * @param throwsList {@code non-null;} list of possibly-thrown exceptions, just used in
     ...
     * {@inheritDoc}
     *
     * <p><b>Note:</b> This compares the method constants only, ignoring any associated code,
     * because it should never be the case that two different items with the same method constant
    ...
    public int encode(DexFile file, AnnotatedOutput out, int lastIndex, int dumpSeq) {
        ...
        if (out.annotates()) {
            ...
            out.annotate(
                    Leb128.unsignedLeb128Size(codeOff), "    code_off:     " + Hex.u4(codeOff));
        }
