...
public final class DexMerger {
    ...
    public DexMerger(Dex dexA, Dex dexB, CollisionPolicy collisionPolicy) throws IOException {
        this(dexA, dexB, collisionPolicy, new WriterSizes(dexA, dexB));
    }

    private DexMerger(Dex dexA, Dex dexB, CollisionPolicy collisionPolicy, WriterSizes writerSizes)
            throws IOException {
        this.dexA = dexA;
        this.dexB = dexB;
    ...
    public Dex merge() throws IOException {
        ...
        if (wastedByteCount > +compactWasteThreshold) {
            DexMerger compacter =
                    new DexMerger(dexOut, new Dex(0), CollisionPolicy.FAIL, compactedSizes);
    ...
    /**
     * Reads an IDs section of two dex files and writes an IDs section of a merged dex file.
     * Populates maps from old to new indices in the process.
     */
    abstract class IdMerger<T extends Comparable<T>> {
        ...
        /** Merges already-sorted sections, reading only two values into memory at a time. */
     ...
     *
     * <p>TODO: this may write multiple copies of the same annotation set. We should shrink the
     * output by merging rather than unioning
     */
     ...
     *       because we can't predict the amount of duplication between dex files, nor can we
     *       predict the length of ULEB-encoded offsets or indices.
