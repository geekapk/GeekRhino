...
public final class BasicBlock implements LabeledItem {
     ...
     *     {@code -1} if this block has no successors (that is, it exits the function/method or is
        ...
        for (int i = sz - 2; i >= 0; i--) {
            ...
            if (one.getBranchingness() != Rop.BRANCH_NONE) {
                throw new IllegalArgumentException(
                        "insns[" + i + "] is a " + "branch or can throw");
            }
        }
     ...
     * {@inheritDoc}
     *
     * <p>Return the identity hashcode of this instance. This is proper, since instances of this
     * class compare by identity (see {@link #equals}).
     */
    @Override
    public int hashCode() {
     ...
     *
     * @return {@code >= -1;} the primary successor, or {@code -1} if this block has no successors
     *     at all
     */
    ...
    /**
     * Gets the secondary successor of this block. It is only valid to call this method on blocks
     * that have exactly two successors.
     *
    ...
    /**
     * Returns whether this block has any associated exception handlers. This is just a shorthand
     * for inspecting the last instruction in the block to see if it could throw, and if so, whether
     * it in fact has any associated handlers.
     *
     ...
     * shorthand for inspecting the last instruction in the block to see if it could throw, and if
     * so, grabbing the catch list out of it. If not, this returns an empty list (not {@code null}).
     *
