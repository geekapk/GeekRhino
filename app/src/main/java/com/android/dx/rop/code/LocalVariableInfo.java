...
public final class LocalVariableInfo extends MutabilityControl {
    ...
    /**
     * {@code non-null;} {@link RegisterSpecSet} to use when indicating a block that has no locals;
     * it is empty and immutable but has an appropriate max size for the method
     */
    private final RegisterSpecSet emptySet;

    /**
     * {@code non-null;} array consisting of register sets representing the sets of variables
     * already assigned upon entry to each block, where array indices correspond to block labels
     */
    ...
    /**
     * Merges the given register set into the set for the block with the given label. If there was
     * not already an associated set, then this is the same as calling {@link #setStarts}.
     * Otherwise, this will merge the two sets and call {@link #setStarts} on the result of the
     * merge.
     *
     * @param label {@code >= 0;} the block label
     * @param specs {@code non-null;} the register set to merge into the start set for the block
     * @return {@code true} if the merge resulted in an actual change to the associated set
     *     (including storing one for the first time) or {@code false} if there was no change
     */
    ...
    /**
     * Gets the register set associated with the start of the block with the given label. This
     * returns an empty set with the appropriate max size if no set was associated with the block in
     * question.
     *
     * @param label {@code >= 0;} the block label
     ...
     * label. This returns a newly-allocated empty {@link RegisterSpecSet} of appropriate max size
     * if there is not yet any set associated with the block.
     *
     * @param label {@code >= 0;} the block label
     * @return {@code non-null;} the associated register set
     */
     ...
     * have a simple type and the one in the instruction can be an arbitrary {@link TypeBearer}
     * (such as a constant value).
     *
    ...
    /**
     * Helper method, to get the starts for a label, throwing the right exception for range
     * problems.
     *
     * @param label {@code >= 0;} the block label
