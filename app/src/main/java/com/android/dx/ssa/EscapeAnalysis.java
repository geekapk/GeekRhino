...
public class EscapeAnalysis {
     ...
     *
     * <p>TOP - Used for objects that haven't been analyzed yet NONE - Object does not escape, and
     * is eligible for scalar replacement. METHOD - Object remains local to method, but can't be
     * scalar replaced. INTER - Object is passed between methods. (treated as globally escaping
     * since this is an intraprocedural analysis) GLOBAL - Object escapes globally.
     */
    ...
    /**
     * Creates a link in the lattice between two EscapeSets due to a put instruction. The object
     * being put is the child and the object being put into is the parent. A child set must always
     * have an escape state at least as high as its parent.
     *
    ...
    /**
     * Merges all links in the lattice among two EscapeSets. On return, the newNode will have its
     * old links as well as all links from the oldNode. The oldNode has all its links removed.
     *
    ...
    /**
     * Process a single instruction, looking for new objects resulting from move result or move
     * param.
     *
    ...
    /**
     * Replaces the instructions that define an array with equivalent registers. For each entry in
     * the array, a register is created, initialized to zero. A mapping between this register and
     * the corresponding array index is added.
     *
    ...
    private void replaceDef(
            SsaInsn def, SsaInsn prev, int length, ArrayList<RegisterSpec> newRegs) {
        ...
        switch (use.getOpcode().getOpcode()) {
            case RegOps.AGET:
                ...
                if (index < length) {
                    ...
                    insertPlainInsnBefore(
                            next, RegisterSpecList.make(source), result, RegOps.MOVE, null);
                } else {
            ...
            case RegOps.APUT:
                ...
                if (index < length) {
                    ...
                    insertPlainInsnBefore(
                            use, RegisterSpecList.make(source), result, RegOps.MOVE, null);
    ...
    /**
     * Identifies extra moves added by scalar replacement and propagates the source of the move to
     * any users of the result.
     */
    ...
    private void insertExceptionThrow(
        ...
        insertPlainInsnBefore(
                newInsn, RegisterSpecList.EMPTY, newReg, RegOps.MOVE_RESULT_PSEUDO, null);
    ...
    private void insertPlainInsnBefore(
        ...
        if (cst == null) {
            newRopInsn =
                    new PlainInsn(newRop, originalRopInsn.getPosition(), newResult, newSources);
        } else {
            newRopInsn =
                    new PlainCstInsn(
                            newRop, originalRopInsn.getPosition(), newResult, newSources, cst);
        }
    ...
    private void insertThrowingInsnBefore(
        ...
        if (cst == null) {
            newRopInsn =
                    new ThrowingInsn(
                            newRop, origRopInsn.getPosition(), newSources, StdTypeList.EMPTY);
        } else {
            newRopInsn =
                    new ThrowingCstInsn(
