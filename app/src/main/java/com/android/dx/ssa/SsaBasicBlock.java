...
public final class SsaBasicBlock {
    ...
    /**
     * the number of moves added to the end of the block during the phi-removal process. Retained
     * for subsequent move scheduling.
     */
    ...
    /**
     * contains last computed value of reachability of this block, or -1 if reachability hasn't been
     * calculated yet
     */
    ...
    /**
     * Adds a phi insn to the beginning of this block. The result type of the phi will be set to
     * void, to indicate that it's currently unknown.
     *
    ...
    /**
     * @return {@code non-null;} the (mutable) instruction list for this block, with phi insns at
     *     the beginning
     */
     ...
     * current successor block ({@code B}). The new block will replace B as A's successor and A as
     * B's predecessor. A and B will no longer be directly connected. If B is listed as a successor
     ...
     * com.android.dx.ssa.SsaMethod#makeExitBlock}
     *
    ...
    /**
     * Adds {@code regV} to the live-out list for this block. This is called by the liveness
     * analyzer.
     *
    ...
    /**
     * Adds {@code regV} to the live-in list for this block. This is called by the liveness
     * analyzer.
     *
    ...
    /**
     * Sorts move instructions added via {@code addMoveToEnd} during phi removal so that results
     * don't overwrite sources that are used. For use after all phis have been removed and all calls
     * to addMoveToEnd() have been made.
     *
     * <p>This is necessary because copy-propogation may have left us in a state where the same
     * basic block has the same register as a phi operand and a result. In this case, the register
     * in the phi operand always refers value before any other phis have executed.
     */
        ...
        if (movesFromPhisAtBeginning > 1) {
            ...
            if (firstNonPhiMoveInsn.isMoveException()) {
                if (true) {
                     ...
                     */
                    throw new RuntimeException(
                            "Unexpected: moves from " + "phis before move-exception");
                } else {
                    /*
                    ...
                    if (!moveExceptionInterferes) {
                        ...
                        int spareRegister =
                                parent.borrowSpareRegister(originalResultSpec.getCategory());
