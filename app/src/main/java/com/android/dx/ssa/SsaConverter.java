...
public class SsaConverter {
    ...
    /**
     * Updates an SSA representation, placing phi functions and renaming all registers above a
     ...
     *
     * <p>Converts CFG into "edge-split" form, such that each node either a unique successor or
     * unique predecessor.
     *
     * <p>In addition, the SSA form we use enforces a further constraint, requiring each block with
     * a final instruction that returns a value to have a primary successor that has no other
     * predecessor. This ensures move statements can always be inserted correctly when phi
    ...
    /**
     * In ROP form, move-exception must occur as the first insn in a block immediately succeeding
     * the insn that could thrown an exception. We may need room to insert move insns later, so make
     * sure to split any block that starts with a move-exception such that there is a unique
