...
public final class SsaMethod {
    ...
    public static SsaMethod newFromRopMethod(
            RopMethod ropMethod, int paramWidth, boolean isStatic) {
     ...
     *
     * @param ropMethod {@code non-null;} the original rop-form method that this instance is based
     *     on
     * @param paramWidth the total width, in register-units, of the method's parameters
     * @param isStatic {@code true} if this method has no {@code this} pointer argument
     */
    ...
    /**
     * Builds an IntList of block indices from a basic block list and a list of labels taken from
     * Rop form.
     *
    ...
    /**
     * Creates an exit block and attaches it to the CFG if this method exits. Methods that never
     * exit will not have an exit block. This is called after edge-splitting and phi insertion,
     * since the edges going into the exit block should not be considered in those steps.
     */
    ...
    /**
     * Returns the count of reachable blocks in this method: blocks that have predecessors (or are
     * the start block)
     *
    ...
    /**
     * Computes reachability for all blocks in the method. First clears old values from all blocks,
     * then starts with the entry block and walks down the control flow graph, marking all blocks it
     * finds as reachable.
     */
    ...
    /**
     * Checks to see if the given SSA reg is ever associated with a local local variable. Each SSA
     * reg may be associated with at most one local var.
     *
    ...
    /**
     * Visits blocks in dom-tree order, starting at the current node. The {@code parent} parameter
     * of the Visitor.visitBlock callback is currently always set to null.
     *
        ...
        for (SsaBasicBlock block : getBlocks()) {
            ...
            if (block != getExitBlock()
                    && (insnsSz == 0
                            ...
                            || lastInsn.getOriginalRopInsn().getOpcode().getBranchingness()
                                    == Rop.BRANCH_NONE)) {
                // We managed to eat a throwable insn

                Insn gotoInsn =
                        new PlainInsn(
                                Rops.GOTO, SourcePosition.NO_INFO, null, RegisterSpecList.EMPTY);
    ...
    /**
     * Sets "back-convert mode". Set during back-conversion when registers are about to be mapped
     * into a non-SSA namespace. When true, use and def lists are unavailable.
     */
