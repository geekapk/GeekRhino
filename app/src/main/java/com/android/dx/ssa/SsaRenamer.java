...
public class SsaRenamer implements Runnable {
    ...
    private class BlockRenamer implements SsaInsn.Visitor {
        ...
        /**
         * contains the set of moves we need to keep to preserve local var info. All other moves
         * will be deleted.
         */
        ...
        /**
         * Provides a register mapping between the old register space and the current renaming
         * mapping. The mapping is updated as the current block's instructions are processed.
         */
        ...
        /**
         * Renames all the variables in this block and inserts appriopriate phis in successor
         * blocks.
         */
        public void process() {
            /*
         ...
         *
         * <ol>
         *   <li>Ensures that all new SSA registers specs in the mapping table with the same
         *       register number are identical. In effect, once an SSA register spec has received or
         *       lost a local variable name, then every old-namespace register that maps to it
         *       should gain or lose its local variable name as well.
         ...
         *       local variable. When {@code currentMapping} is updated and the newly added element
         ...
         * they represent a local variable assignment. If they represent a local variable
         * assignement, they are preserved.
         */
            ...
            if (!Optimizer.getPreserveLocals()
                ...
                RegisterSpecList ssaSources =
                        RegisterSpecList.make(
                                RegisterSpec.make(ssaReg.getReg(), ssaReg.getType(), newLocal));

                SsaInsn newInsn =
                        SsaInsn.makeFromRop(
                                new PlainInsn(
                                        Rops.opMarkLocal(ssaReg),
         ...
         * {@inheritDoc}
         *
         * <p>All insns that are not move or phi insns have their source registers mapped ot the
         * current mapping. Their result registers are then renamed to a new SSA register which is
         * then added to the current register mapping.
         */
        ...
        /**
         * Updates the phi insns in successor blocks with operands based on the current mapping of
         * the rop register the phis represent.
         */
