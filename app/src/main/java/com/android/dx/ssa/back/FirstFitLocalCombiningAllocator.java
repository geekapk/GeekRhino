...
public class FirstFitLocalCombiningAllocator extends RegisterAllocator {
    ...
    private enum Alignment {
        ...
        /**
         * Returns the index of the first bit that is set to {@code false} that occurs on or after
         * the specified starting index and that respect {@link Alignment}.
         *
     ...
     * range that's wide enough for all of the SSA registers, and then tries to map them all to that
     * range. If not all fit, a new range is tried until all registers have been fit.
     */
    ...
    /**
     * Returns true if given rop register represents the {@code this} pointer for a non-static
     * method.
     *
    ...
    /**
     * Maps any parameter that isn't local-associated, which can happen in the case where there is
     * no java debug info.
     */
    ...
    /**
     * Checks to see if {@code ssaSpec} can be mapped to {@code ropReg}. Checks interference graph
     * and ensures the range does not cross the parameter range.
     *
    ...
    /**
     * Returns true if the specified rop register + category will cross the boundry between the
     * lower {@code paramWidth} registers reserved for method params and the upper registers. We
     * cannot allocate a register that spans the param block and the normal block, because we will
     * be moving the param block to high registers later.
     *
    ...
    private void analyzeInstructions() {
        ssaMeth.forEachInsn(
                new SsaInsn.Visitor() {
                     ...
                     *
                     * <p>1) Adds a local variable assignment to the {@code localVariables} map. 2)
                     * Add move-result-pseudo to the {@code moveResultPseudoInsns} list. 3) Add
                     * invoke-range to the {@code invokeRangeInsns} list.
                     *
                     * @param insn {@code non-null;} insn that may represent a local variable
                     *     assignment
                     */
    ...
    /**
     * Maps the source registers of the specified instruction such that they will fall in a
     * contiguous range in rop form. Moves are inserted as necessary to allow the range to be
     * allocated.
     *
     ...
     * try to center the range around sources that have already been mapped to rop registers. If
     * that fails, just find a new contiguous range that doesn't interfere.
     *
     ...
     *     source
     * @param outMovesRequired {@code non-null;} an output parameter indexed by source index that
     *     will contain the set of sources which need moves inserted
    ...
    /**
     * Converts a bit set of SSA registers into a RegisterSpecList containing the definition specs
    ...
    private static class Multiset {
         ...
         * element that was added first is returned. Then, it clears the count on that element. The
         * size of the set remains unchanged.
         *
