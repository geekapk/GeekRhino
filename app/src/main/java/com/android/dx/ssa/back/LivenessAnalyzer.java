...
public class LivenessAnalyzer {
     ...
     * with each other. This is needed since the dead code remover has allowed through "dead-end
     * phis" whose results are not used except as local assignments. Without this step, a the result
     * of a dead-end phi might be assigned the same register as the result of another phi, and the
     * phi removal move scheduler may generate moves that over-write the live result.
     *
        ...
        for (SsaBasicBlock b : ssaMeth.getBlocks()) {
            ...
            for (int i = 0; i < szPhis; i++) {
                for (int j = 0; j < szPhis; j++) {
                    ...
                    interference.add(
                            phis.get(i).getResult().getReg(), phis.get(j).getResult().getReg());
                }
            }
        }
    }
