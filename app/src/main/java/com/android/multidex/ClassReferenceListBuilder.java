 ...
 * directories, they constitute in a classpath in with the classes named by the first argument will
 * be searched. Each searched class must be found. On each of this classes are searched for their
 * dependencies to other classes. Finally the tools prints on standard output a list of class files
 * names suitable as content of the file argument --main-dex-list of dx.
 */
public class ClassReferenceListBuilder {
    ...
    private static ClassPathElement getClassPathElement(File file)
        ...
        if (file.isDirectory()) {
            ...
            throw new IOException(
                    file.getAbsolutePath() + " is not a directory neither a zip file");
        } else {
                ...
                if (descriptor.endsWith(";")) {
                    ...
                    if (lastBrace < 0) {
                        ...
                        addClassWithHierachy(
                                descriptor.substring(lastBrace + 2, descriptor.length() - 1));
                    }
                }
            }
        }
    ...
    private static class Path {
            ...
            for (String filePath : definition.split(Pattern.quote(File.pathSeparator))) {
                try {
                    ...
                    throw new IOException(
                            "\""
                                    + filePath
                                    + "\" can not be used as a classpath"
                                    + " element. ("
                                    + e.getMessage()
                                    + ")",
                            e);
        ...
        private static byte[] readStream(
                InputStream in, ByteArrayOutputStream baos, byte[] readBuffer) throws IOException {
            try {
                for (; ; ) {
