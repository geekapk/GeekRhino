...
abstract class BaseAndroidClassLoader extends ClassLoader implements GeneratedClassLoader {
    ...
    public Class<?> defineClass(String name, byte[] data) {
        try {
            ...
            com.android.dx.dex.file.DexFile dexFile =
                    new com.android.dx.dex.file.DexFile(dexOptions);
            DirectClassFile classFile =
                    new DirectClassFile(data, name.replace('.', '/') + ".class", true);
            classFile.setAttributeFactory(StdAttributeFactory.THE_ONE);
            classFile.getMagic();
            dexFile.add(
                    CfTranslator.translate(classFile, null, new CfOptions(), dexOptions, dexFile));
    ...
    protected abstract Class<?> loadClass(@NonNull Dex dex, @NonNull String name)
    ...
    /**
     * Try to load a class. This will search all defined classes, all loaded jars and the parent
     * class loader.
     *
